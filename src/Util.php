...
class Util
{
     ...
     */
    public static function formInput(bool $append_session = false): string
    {
     ...
     * Util::_deleteAtShutdown() as a shutdown function - no need to do
     * so manually.
     *
     ...
     * shutdown function by Util::deleteAtShutdown() and called
     ...
     */
    protected static function _secureDelete($file, array $ptr = [])
    {
        if (isset($ptr['secure'][$file])) {
            $filesize = filesize($file);
            $fp = fopen($file, 'r+');
            foreach (self::$patterns as $pattern) {
                $pattern = substr(str_repeat($pattern, (int) floor($filesize / strlen($pattern)) + 1), 0, $filesize);
                fwrite($fp, $pattern);
                fseek($fp, 0);
            }
            fclose($fp);
